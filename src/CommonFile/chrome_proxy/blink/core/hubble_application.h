...
class CORE_EXPORT HubbleApplication final : public EventTargetWithInlineData,
									  public DOMWindowClient {
  DEFINE_WRAPPERTYPEINFO();
  USING_GARBAGE_COLLECTED_MIXIN(HubbleApplication);

 public:
  static HubbleApplication* Create(LocalFrame* frame) { return MakeGarbageCollected<HubbleApplication>(frame); }
  static HubbleApplication* Create(LocalFrame* frame, const String& strHandle);
  ...
  DEFINE_ATTRIBUTE_EVENT_LISTENER(AppMessageReceived, kHubbleapplication)

  // EventTarget overrides:
  const AtomicString& InterfaceName() const override;
  ExecutionContext* GetExecutionContext() const override;

  explicit HubbleApplication(LocalFrame*);
  HubbleApplication(LocalFrame*, const String& strNodeXml);

  ~HubbleApplication() override;

...
namespace TangramCommon
{
	...
	CHubbleDocProxy::CHubbleDocProxy()
	{
	...
	CHubbleDocHelper::CHubbleDocHelper()
	{
		m_pDoc				= nullptr;
		m_pDocument			= nullptr;
		m_strHubbleData	= _T("");
	...
	void CHubbleDocHelper::TangramSerialize(CDocument* _pDoc, CArchive* pAr)
	{
	...
	BOOL CTangramViewHelper::CreateView(CCreateContext* pContext, HWND hView, HWND hParent)
	{
		if (::IsWindow(hView) && g_pHubble)
		{
			...
			{
				m_pDoc = (IHubbleDoc*)it->second;
			}
			else
			{
				m_pDoc = (IHubbleDoc*)theAppImpl.m_pUniverseAppProxy->m_pHubbleImpl->ConnectHubbleDoc((IUniverseAppProxy*)theAppImpl.m_pUniverseAppProxy, (LONGLONG)pContext->m_pCurrentDoc, hView, hParent, m_strDocID);
	...
	BOOL IUniverseAppProxy::InitHubble(void* pVoid)
	{
		...
		if (hModule) {
			...
			GetHubbleImpl _pHubbleFunction2;
			_pHubbleFunction2 = (GetHubbleImpl)GetProcAddress(hModule, "GetHubbleImpl");
	...
	IHubbleDoc* IUniverseAppProxy::NewDoc()
	{
		CDocument* pDoc = NULL;
